...
void CTPConnection::OnRtnDepthMarketData(CThostFtdcDepthMarketDataField *pDepthMarketData)
{
    ...
    market_data.AddMember("connection_id", rapidjson::StringRef(config_.connection_id.c_str()), allocator);
        ...
        try {
            ...
            for (const auto& conn : connections_to_check) {
                ...
                if (status == CTPConnectionStatus::ERROR || 
                    ...
                    server_->log_warning("Connection " + conn->get_connection_id() + " is unhealthy, attempting restart");
